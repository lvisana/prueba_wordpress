...
class es_cls_registerhook {
	...
	public static function es_get_form( $instance ) {
        ...
        <div class="es_form_container">
			<h2 class="tertiary-font header-para mb-4 fs-1"> <?php echo $es_title ?></h2>
            <form class="es_<?php echo $es_pre ?>_form" data-es_form_id="es_<?php echo $es_pre ?>_form">
				<?php if ( $es_desc != "" ) { ?>
                    <div class="text-uppercase fs-2 lh-sm es_caption"><?php echo $es_desc; ?></div>
				<?php } ?>
				<?php if ( $es_name == "YES" ) { ?>
				...
				<?php } ?>
                <div class="es_lablebox">
                    <label class="d-none es_<?php echo $es_pre ?>_form_email"><?php echo __( 'Email *', 'email-subscribers' ); ?></label>
                </div>
                <div class="input-group center-container" id="newsletter">
					<div class="p-0 es_textbox">
						<input aria-label="Your Email" aria-describedby="basic-addon2" type="email" placeholder="Your Email" id="es_txt_email" class="py-2 form-control es_textbox_class" name="es_txt_email" value="" maxlength="60" required>
					</div>
									<?php if ( ( in_array( 'gdpr/gdpr.php', $active_plugins ) || array_key_exists( 'gdpr/gdpr.php', $active_plugins ) ) ) {
										echo GDPR::consent_checkboxes();
									} ?>
					<div class="input-group-append es_button p-0">
						<input type="submit" id="es_txt_button" class="px-4 py-2 secondary-light-para btn tertiary fw-light es_textbox_button es_submit_button" name="es_txt_button" value="<?php echo __( 'Subscribe', 'email-subscribers' ); ?>">
					</div>
				</div>
...
class es_widget_register extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( ! empty( $es_title ) ) {
			$args['before_title'] . $es_title . $args['after_title'];
